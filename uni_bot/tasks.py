...
def notify_bot_about_updated_course(course_id: str) -> None:
    """
    Notify the bot about a course content update (publish).
    """
    signal_data = {'course_id': course_id, 'event_type': 'updated'}

    response = UniBotCourseContextClient().send_course_update_signal(course_id, signal_data)

    try:
        response.raise_for_status()
        logger.info('A signal about course %s update is successfully sent to the bot.', course_id)
    except requests.exceptions.HTTPError:
        message = response.content.decode('utf-8')
        logger.info('Failed to send a signal about course %s update to the bot: "%s".', course_id, message)
